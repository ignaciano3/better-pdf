...
- **P3.** ✅ **Done.** Converted all 51 `buf.extend_from_slice(format!(…).as_bytes())` sites (draw.rs/appearance.rs/create.rs) to `write!(buf, …)` / `writeln!(buf, …)` — `Vec<u8>: io::Write`, so each call drops a throwaway heap `String` (matters in per-glyph/per-line/per-op loops) and trims a little WASM code size. Byte output is identical (same Display impls; `writeln!` to a `Vec` appends `\n`). Clippy is clean under CI's `-D warnings`. `fmt_num` still returns a `String` per number — left as a follow-up (would need a `Display` newtype across ~100 call sites). Verified: cargo 284 / bun 261 / clippy green; roundtrip + qpdf byte-validation tests pass.

## Tier 2 — God-function / god-class splits (structural; after Tier 0 helpers exist) — ✅ ALL DONE

- **P4.** ✅ **Done.** Split `create_document_json` (was ~1,425 lines) into a thin coordinator (~175 lines) + three extracted functions: `validate_create` (both op- and field-validation passes, pure/read-only), `build_pages` (per-page content streams + page tree, returns `page_ids`), and `build_fields_and_acroform` (AcroForm + widget annotations, returns `Option<acro_id>`). Outline/info already delegate to their own modules, so the coordinator just calls them. Logic moved verbatim (no behavior/output-byte change); only the moved-into-a-function binding modes changed (`&ops`→`ops` for slice params, one `&mut *doc` reborrow). Verified: cargo 284 / bun 261 / clippy green against fresh WASM.
- **P5.** ✅ **Done.** Split `draw_apply` (the ~735-line `apply_draw_ops_json` core) into a thin coordinator + three functions: `validate_draw_ops` (font-range + per-op validation, read-only), `embed_fonts` (subset/Type0 build pre-pass, returns the `id → (type0_id, BuiltFont)` map), and `emit_page_ops` (the per-page content-stream emit + `/Contents` splice). `draw_apply` now reads validate → group-by-page → embed → emit. Logic moved verbatim (no behavior/output-byte change); only the `&page_ops` slice-param binding mode changed. No rustfmt run (draw.rs isn't canonical), so the diff is just the relocation. Verified: cargo 284 / bun 261 / clippy green against fresh WASM. (The deeper "shared content/ops module across create & draw" remains a possible follow-up.)
- **P6.** ✅ **Done (API-preserving subset).** Extracted two cohesive collaborators that `PdfDocumentBase` now delegates to: `MetadataState` (new `src/core/metadata-state.ts` — the wire map + dirty flag, the 8 typed setters, and the read-overlay `merge()`), and a pure module-level `buildPageIndexResolver(structureOps, loadedCount)` function (the instance method is now a 3-line wrapper). Behavior and the public API are unchanged. **Deliberately NOT done:** the `CreatedDocument` vs `LoadedDocument` subclass split — `PdfDocument` is a single stable 1.0 type whose `load()` and `create()` both return it, so splitting it into two subclasses would be a breaking public-API change, which violates this plan's no-API-change constraint. `ResourceEmbedder` was skipped as low-value (its methods are thin one-line WASM wrappers). Verified: bun 261 / tsc clean; browser-entry smoke passes end-to-end.
