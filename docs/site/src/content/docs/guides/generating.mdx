...
To mirror an image, pass `flipX` (horizontal) and/or `flipY` (vertical) to
`drawImage`. The flip happens inside the placement box, so `(x, y)` stays the
bottom-left corner and the drawn rectangle is unchanged — and it composes with
`rotate`, mirroring along the image's own axes:

```ts
page.drawImage(logo, { x: 40, y: 700, width: 120, height: 80, flipX: true });
```
...
To work with every page handle at once, `doc.getPages()` returns the full
`PdfPage[]` in document order (the same instances `doc.getPage(i)` returns):

```ts
const pages = doc.getPages();
for (const page of pages) {
  page.drawText("DRAFT", { x: 50, y: 50, size: 24, opacity: 0.2 });
}
```

`doc.getPageCount()` reports the number of pages; `doc.getPage(i)` fetches one
by zero-based index and throws `PageOutOfRangeError` when it is out of range.
